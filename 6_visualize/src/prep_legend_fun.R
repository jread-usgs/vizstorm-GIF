...
prep_legend_fun <- function(precip_bins, legend_styles, storm_points_sf, DateTime=NA){
  ...
  plot_fun <- function(){
    ...
    xright <- right_edge - bin_h/2
    ybottom <- coord_space[3]+ bin_h/2
    ...
    dot_x <- right_edge-bin_w/4
    dot_txt_x <- right_edge-bin_w*0.4

    text(right_edge, precip_txt_y, labels = 'NOAA total rainfall amount (inches)', pos = 2, cex = leg_cex)
    points(dot_x, flood_y, pch = 21, bg = legend_styles$gage_norm_col, col = legend_styles$gage_flood_col, lwd = 4, cex = 2)
    text(dot_txt_x, flood_y, labels = 'Above flood stage', pos = 2, cex = leg_cex)
    points(dot_x, normal_y, pch = 21, bg = legend_styles$gage_norm_col, col = NA, cex = 2)
    text(dot_txt_x, normal_y, labels = 'Below flood stage', pos = 2, cex = leg_cex)
    text(right_edge, gage_caveat_y, labels = 'USGS Stream Gages (< 1% of U.S. total)', pos = 2, cex = leg_cex)
    if (length(hurricane_cat) == 0){
      hurricane_cat <- "Hurrricane %s" #hack for now
      hurricane_col <- 'orange'
    }
    text(dot_txt_x, hurricane_y, labels = sprintf(hurricane_cat, legend_styles$storm_name), pos = 2, cex = leg_cex)
